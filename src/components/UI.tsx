...
export function UI() {
  ...
  useEffect(() => {
    if (status === "success" || status === "crashed") {
      const delay = status === "crashed" ? 6000 : 800;
  ...
  return (
    <div
      {...bind()}
      className="absolute inset-0 z-10 flex flex-col overflow-hidden touch-none"
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 pointer-events-none">
        {status === "menu" && (
          <div className="pointer-events-auto bg-black/80 backdrop-blur-lg p-8 rounded-3xl border border-purple-500/50 flex flex-col items-center text-center max-w-sm w-full shadow-[0_0_50px_rgba(168,85,247,0.2)]">
            ...
            <label className="text-sm font-bold text-slate-400 mb-2 uppercase tracking-wider">
              Game Length
            </label>
          ...
          <div className="w-full max-w-md absolute bottom-24 left-1/2 -translate-x-1/2 px-4 pointer-events-none animate-in fade-in slide-in-from-bottom-10 duration-500">
            ...
            <div className="flex gap-4">
              <div className="flex-1 bg-blue-600/90 backdrop-blur-md p-4 rounded-2xl border-2 border-blue-400/50 shadow-[0_0_30px_rgba(37,99,235,0.4)] flex flex-col justify-center">
                <div className="text-blue-200 text-xs sm:text-sm font-bold mb-1 uppercase tracking-widest text-center">
                  ← Swipe Left
                </div>
                <div className="text-white font-bold text-center text-base sm:text-lg">
              ...
              <div className="flex-1 bg-pink-600/90 backdrop-blur-md p-4 rounded-2xl border-2 border-pink-400/50 shadow-[0_0_30px_rgba(219,39,119,0.4)] flex flex-col justify-center">
                <div className="text-pink-200 text-xs sm:text-sm font-bold mb-1 uppercase tracking-widest text-center">
                  Swipe Right →
                </div>
                <div className="text-white font-bold text-center text-base sm:text-lg">
        ...
        {status === "crashed" && (
          <div
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center bg-black/90 backdrop-blur-xl p-6 rounded-[2.5rem] border-4 border-red-600 shadow-[0_0_60px_rgba(220,38,38,0.6)] z-50 animate-bounce pointer-events-auto cursor-pointer w-11/12 max-w-sm"
          >
            <div className="relative mb-6 overflow-hidden rounded-3xl border-4 border-white shadow-2xl pointer-events-none">
              <img
                src="/assets/cuza-wrong.jpeg"
            ...
            <div className="text-5xl font-black text-red-500 drop-shadow-[0_0_20px_rgba(239,68,68,0.8)] text-center leading-tight uppercase italic pointer-events-none">
              Ai greșit!
            </div>
            <div className="mt-4 text-red-400/60 text-xs font-bold uppercase tracking-widest animate-pulse pointer-events-none">
              Tap to continue
            </div>
          </div>
        )}
